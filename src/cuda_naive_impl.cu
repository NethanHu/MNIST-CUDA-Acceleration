...
// Naive impl of exception prompt 
...
// Because the data is read to SRAM, so it keeps identical
...
// identical, we will copy the rand weights to CUDA
...
// initialize the weights, bias in the NN
void initialize_neural_network(CUDANeuralNetwork *nn) {
...
// Naive impl calculation of mat_mul on CUDA
...
// remove all the historical grad
...
void forward(CUDANeuralNetwork *nn, float *d_input, float *d_hidden, float *d_output, int batch_size) {
    // 748 threads/blocks (fits the 28 * 28 figure)
    dim3 block_size(28, 28);
    ...
    bias_add_kernel<<<(batch_size * HIDDEN_SIZE + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(d_hidden, nn->bias1, batch_size, HIDDEN_SIZE);
    CUDA_CHECK(cudaGetLastError());

    relu_kernel<<<(batch_size * HIDDEN_SIZE + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(d_hidden, batch_size * HIDDEN_SIZE);
    CUDA_CHECK(cudaGetLastError());
    ...
    bias_add_kernel<<<(batch_size * OUTPUT_SIZE + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(d_output, nn->bias2, batch_size, OUTPUT_SIZE);
    CUDA_CHECK(cudaGetLastError());
...
// calculation of loss function (on CPU)
...
__global__ void compute_output_gradients_kernel(float *grad_output, float *output, int *labels, int batch_size) {
    ...
    if (b < batch_size) {
        ...
        // True label at labels[b], only minus this term by 1.0
...
__global__ void update_gradients_kernel(float *grad_weights, float *grad_bias, float *grad_layer, float *prev_layer, int batch_size, int prev_size, int curr_size) {
    ...
    if (i < curr_size && j < prev_size) {
        ...
        // only if all other threads finishes, the update begins
        atomicAdd(&grad_weights[i * prev_size + j], grad_w_sum);
        // an optimizaion skill
        if (j == 0) {
...
void backward(CUDANeuralNetwork *nn, float *d_input, float *d_hidden, float *d_output, int *d_labels, int batch_size) {
    zero_grad_kernel<<<(HIDDEN_SIZE * INPUT_SIZE + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(nn->grad_weights1, HIDDEN_SIZE * INPUT_SIZE);
    CUDA_CHECK(cudaGetLastError());
    zero_grad_kernel<<<(OUTPUT_SIZE * HIDDEN_SIZE + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(nn->grad_weights2, OUTPUT_SIZE * HIDDEN_SIZE);
    CUDA_CHECK(cudaGetLastError());
    zero_grad_kernel<<<(HIDDEN_SIZE + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(nn->grad_bias1, HIDDEN_SIZE);
    CUDA_CHECK(cudaGetLastError());
    zero_grad_kernel<<<(OUTPUT_SIZE + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(nn->grad_bias2, OUTPUT_SIZE);
    CUDA_CHECK(cudaGetLastError());
    
    // Compute gradients for output layer
    float *d_grad_output;
    CUDA_CHECK(cudaMalloc(&d_grad_output, batch_size * OUTPUT_SIZE * sizeof(float)));
    compute_output_gradients_kernel<<<(batch_size + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(d_grad_output, d_output, d_labels, batch_size);
    CUDA_CHECK(cudaGetLastError());
    ...
    dim3 grid_size_w2((OUTPUT_SIZE + block_size.x - 1) / block_size.x, (HIDDEN_SIZE + block_size.y - 1) / block_size.y);
    matmul_at_b_kernel<<<grid_size_w2, block_size>>>(d_hidden, d_grad_output, nn->grad_weights2, batch_size, HIDDEN_SIZE, OUTPUT_SIZE);
    CUDA_CHECK(cudaGetLastError());
    ...
    drelu_kernel<<<(batch_size * HIDDEN_SIZE + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(d_hidden, d_grad_hidden, batch_size * HIDDEN_SIZE);
    CUDA_CHECK(cudaGetLastError());
    multiply_gradients_kernel<<<(batch_size * HIDDEN_SIZE + HIDDEN_SIZE - 1) / HIDDEN_SIZE, HIDDEN_SIZE>>>(d_dX2, d_grad_hidden, batch_size * HIDDEN_SIZE);
    CUDA_CHECK(cudaGetLastError());
    ...
    dim3 grid_size_w1((HIDDEN_SIZE + block_size.x - 1) / block_size.x, (INPUT_SIZE + block_size.y - 1) / block_size.y);
    matmul_at_b_kernel<<<grid_size_w1, block_size>>>(d_input, d_dX2, nn->grad_weights1, batch_size, INPUT_SIZE, HIDDEN_SIZE);
    CUDA_CHECK(cudaGetLastError());
...
void update_weights(CUDANeuralNetwork *nn) {
    int block_size = 256;
    int grid_size;
...
// test step in CPU
float evaluate_accuracy(CUDANeuralNetwork *nn, float *d_X_test, int *d_y_test, float *d_hidden, float *d_output, int total_size) {
...
// training process in CUDA
void train(CUDANeuralNetwork *nn, float *X_train, int *y_train, float *X_test, int *y_test) {
...
int main() {
    ...
    printf("=== Naive CUDA Neural Network Configuration ===\n");
    printf("      Input Size:        %d\n", INPUT_SIZE);
