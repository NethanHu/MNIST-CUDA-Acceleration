...
#define TRAIN_SIZE 10000 // You can add it to 60000 (much slower)
#define TEST_SIZE 1000   // You can add it to 10000 (much slower)
#define BATCH_SIZE 4
#define EPOCHS 3
...
// function that is used for calculate grad
...
/* A naive implementation of MLP in C:
 ...
 * Apply softmax
 */
void forward(CPUNeuralNetwork *nn, float *input, float *hidden, float *output, int batch_size) {
...
// implementation of bias grad (constant)
...
// update grad for weights and biases
...
/* A naive implementation of back propagation in C:
 * Remove all the historical grad
 * Compute grad for output layer
 * Update grad for weights2 (W2.grad = grad_output.T @ hidden)
 * Update grad for bias2
 ...
 */
void backward(CPUNeuralNetwork *nn, float *input, float *hidden, float *output, int *labels, int batch_size) {
...
// gradient descent
void update_weights(CPUNeuralNetwork *nn) {
...
void train(CPUNeuralNetwork *nn, float *X_train, int *y_train) {
...
void initialize_neural_network(CPUNeuralNetwork *nn) {
...
int main() {
    ...
    printf("=== CPU Neural Network Configuration ===\n");
    printf("      Input Size:        %d\n", INPUT_SIZE);
    ...
    printf("Start naive training on CPU...\n");
